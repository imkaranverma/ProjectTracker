...
function Task() {
    ...
    useEffect(() => {
        if (!isAddTaskModalOpen || isRenderChange) {
            axios.get(`https://projecttracker-1.onrender.com/project/${projectId}`)
                .then((res) => {
    ...
    const updateTodo = (data) => {
        axios.put(`https://projecttracker-1.onrender.com/project/${projectId}/todo`, data)
            .then((res) => {
            }).catch((error) => {
                toast.error('Something went wrong')
            })
    ...
    const handleDelete = (e, taskId) => {
        e.stopPropagation();
        axios.delete(`https://projecttracker-1.onrender.com/project/${projectId}/task/${taskId}`)
            .then((res) => {
